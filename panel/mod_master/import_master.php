...
if (isset($_FILES['file']['name'])) {
    ...
    if (in_array($extension, $ext)) {
        ...
        for ($i = 1; $i < count($sheetData); $i++) {
            ...
            if ($nama <> '') {
                $exec = mysqli_query($koneksi, "INSERT INTO siswa (id_siswa,id_kelas,idpk,nis,no_peserta,nama,level,sesi,ruang,username,password,foto,server,agama,hp,tempat_lahir,alamat) VALUES ('$id_siswa','$kelas','$pk','$nis','$no_peserta','$nama','$level','$sesi','$ruang','$username','$password','$foto','$server','$agama','$nohp','$ttl','$alamat')");
                ($exec) ? $sukses++ : $gagal++;
            }
        }
